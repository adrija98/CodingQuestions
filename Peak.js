...
// By using divide and conquer (iteratively) => Binary search method. It will always find a peak.
// Time complexity: O(logn)
// Space complexity: O(1)
const findPeakIndex2 = (arr) => {
    let left = 0;
...
// Time complexity: O(logn)
// Space complexity: O(1)
const findPeakIndex3 = (arr, left = 0, right = arr.length - 1) => {
    if(left >= right) 
        return left;
    let mid = Math.floor((left + right)/2);
    if(arr[mid] < arr[mid+1])
...
console.log(findPeakIndex1([8, 2]));